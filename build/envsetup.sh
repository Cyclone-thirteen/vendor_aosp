...
function pixelgerrit() {
    ...
    local user=`git config --get review.gerrit.cyclone.org.username`
...
function pixelrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "Cyclone Gerrit Rebase Usage: "
    ...
    git fetch "http://gerrit.cyclone.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
